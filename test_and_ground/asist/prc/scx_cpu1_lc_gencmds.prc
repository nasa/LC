PROC scx_cpu1_lc_gencmds
;*******************************************************************************
;  Test Name:  lc_gencmds
;  Test Level: Build Verification
;  Test Type:  Functional
;
;  Test Description
;	The purpose of this test is to verify that Limit Checker (LC)
;	application general commands function properly. All LC commands will be
;	tested as well as invalid commands and an application reset to see if
;	the LC application behaves appropriately
;
;  Requirements Tested
;     LC1000	Upon receipt of a No-Op command, LC shall increment the LC
;		Valid Command Counter and generate an event message.
;     LC1001	Upon receipt of a Reset command, LC shall reset the following
...
;     LC1002	For all LC commands, if the length contained in the message
...
;		event message.
;     LC1004	If LC rejects any command, LC shall abort the command
;		execution, increment the LC Command Rejected Counter and issue
;		an event message
...
;     LC4009.1	If a command is received to Activate an Actionpoint which has
;		been permanently disabled, the command shall be rejected.
;     LC4010	Upon receipt of a Reset Actionpoint Statistics command, LC
...
;		      Actionpoint
;     LC4011	Upon receipt of a Reset All Actionpoint Statistics command, LC
;		shall set to zero, all the following Actionpoint Statistics for
;		all Actionpoints:
;		   a) Total number of FAIL to PASS transitions
;		   b) Total number of PASS to FAIL transitions
...
;		   f) Total number of event messages sent relating to that
;		      Actionpoint
;     LC4012	Upon receipt of a Reset Watchpoint Statistics command, LC shall
...
;     LC4013	Upon receipt of a Reset All Watchpoint Statistics command, LC
...
local cfe_requirements[0 .. ut_req_array_size] = ["LC_1000","LC_1001", ;;
    "LC_1002","LC_1003","LC_1004","LC_2004","LC_3006","LC_4000", ;;
    "LC_4001","LC_4002","LC_4003","LC_4004","LC_4005","LC_4006", ;;
    "LC_4007","LC_4008","LC_4009","LC_4009.1","LC_4010","LC_4011", ;;
    "LC_4012","LC_4013","LC_8000","LC_9000","LC_9001","LC_9002"]
...
page SCX_CPU1_LC_HK
page SCX_CPU1_TST_LC_HK
page SCX_CPU1_LC_ART
page SCX_CPU1_LC_WRT
...
ut_setupevents "SCX", "CPU1", {LCAppName}, LC_INIT_INF_EID, "INFO", 2
...
write ";**********************************************************************"
ut_setupevents "SCX", "CPU1", "CFE_ES", CFE_ES_START_INF_EID, "INFO", 1
ut_setupevents "SCX", "CPU1", "TST_LC", TST_LC_INIT_INF_EID, "INFO", 2
...
if (SCX_CPU1_LC_CMDPC = 0) AND (SCX_CPU1_LC_CMDEC = 0) AND ;;
   (SCX_CPU1_LC_PASSRTSCNT = 0) AND ;;
   (SCX_CPU1_LC_CURLCSTATE=LC_STATE_POWER_ON_RESET) AND ;;
   (SCX_CPU1_LC_APSAMPLECNT = 0) AND (SCX_CPU1_LC_MONMSGCNT = 0) AND ;;
   (SCX_CPU1_LC_RTSCNT = 0) THEN
;; 255 because the WPs are all not measured   
  for wpindex = 1 to WPACKED do
    if (SCX_CPU1_LC_WRRESULTS[wpindex].WPByteEntry.AllStatus <> 255) then
      break
    endif
  enddo
...
;;255 is because they are disabled and not measured
  for apindex = 1 to APResults do
    if (SCX_CPU1_LC_ARRESULTS[apindex].APByteEntry.AllStatus <> 255) then
      break                                   
    endif
  enddo
...
;; 51 is because the APs are not used and not measured
  for apindex = APResults+1 to APACKED do
    if (SCX_CPU1_LC_ARRESULTS[apindex].APByteEntry.AllStatus <> 51) then
      break                                   
    endif
  enddo
  ...
  if (wpindex < WPACKED) OR (apindex < APACKED) then
    ...
    write "  CMDPC             = ", SCX_CPU1_LC_CMDPC 
    write "  CMDEC             = ", SCX_CPU1_LC_CMDEC 
    write "  PASSRTSCNT        = ", SCX_CPU1_LC_PASSRTSCNT
    write "  CURLCSTATE        = ", SCX_CPU1_LC_CURLCSTATE
    write "  APSAMPLECNT       = ", SCX_CPU1_LC_APSAMPLECNT 
    write "  MONMSGCNT         = ", SCX_CPU1_LC_MONMSGCNT
    write "  RTSCNT            = ", SCX_CPU1_LC_RTSCNT 
    if (wpindex < WPACKED) then
      write "  WP Packed Results = ", SCX_CPU1_LC_WRRESULTS[wpindex].WPByteEntry.AllStatus
    endif
    if (apindex < APACKED) then
      write "  AP Packed Results = ", SCX_CPU1_LC_ARRESULTS[apindex].APByteEntry.AllStatus
    endif
...
s get_tbl_to_cvt(ramDir, WRTTblName, "A", "cpu1_dumpwrt", hostCPU, wrtAppId)
wait 5

for index = 0 to LC_MAX_WATCHPOINTS-1 do
  if (SCX_CPU1_LC_WRT[index].WatchResults <> LC_WATCH_STALE) or ;;
     (SCX_CPU1_LC_WRT[index].EvaluationCount <> 0) or ;;
     (SCX_CPU1_LC_WRT[index].FalsetoTrueCount <> 0) or ;;
     (SCX_CPU1_LC_WRT[index].ConsectiveTrueCount <> 0) or ;;
     (SCX_CPU1_LC_WRT[index].CumulativeTrueCount <> 0) or ;;
     (SCX_CPU1_LC_WRT[index].FtoTValue.UnSigned32 <> 0) or ;;
     (SCX_CPU1_LC_WRT[index].TtoFValue.UnSigned32 <> 0) then
    break
  endif
enddo
...
if (index < LC_MAX_WATCHPOINTS-1) then
  ...
  write " WatchResults        =", SCX_CPU1_LC_WRT[index].WatchResults
  write " Evaluation Count    =", SCX_CPU1_LC_WRT[index].EvaluationCount
  write " False to True Count =", SCX_CPU1_LC_WRT[index].FalsetoTrueCount
  write " Consecutive True    =", SCX_CPU1_LC_WRT[index].ConsectiveTrueCount
  write " Cum True Count      =", SCX_CPU1_LC_WRT[index].CumulativeTrueCount
  write " F to T Value        =", SCX_CPU1_LC_WRT[index].FtoTValue.UnSigned32
  write " T to F Value        =", SCX_CPU1_LC_WRT[index].TtoFValue.UnSigned32
...
s get_tbl_to_cvt(ramDir, ARTTblName, "A", "cpu1_dumpart", hostCPU, artAppId)
wait 5 

for index = 0 to LC_MAX_ACTIONPOINTS-1 do
  if (index < ADTentries) then
    if  (SCX_CPU1_LC_ART[index].ActionResult <> LC_ACTION_STALE) or ;;
	(SCX_CPU1_LC_ART[index].CurrentState <> LC_APSTATE_DISABLED) or ;;
	(SCX_CPU1_LC_ART[index].FailToPassCount <> 0) or ;;
	(SCX_CPU1_LC_ART[index].PassToFailCount <> 0) or ;;
	(SCX_CPU1_LC_ART[index].ConsecutiveFailCount <> 0) or ;;
	(SCX_CPU1_LC_ART[index].CumulativeFailCount <> 0) or ;;
	(SCX_CPU1_LC_ART[index].CumulativeRTSExecCount <> 0) then
      break
    endif
  else
    if  (SCX_CPU1_LC_ART[index].ActionResult <> LC_ACTION_STALE) or ;;
	(SCX_CPU1_LC_ART[index].CurrentState <> LC_ACTION_NOT_USED) or ;;
	(SCX_CPU1_LC_ART[index].FailToPassCount <> 0) or ;;
	(SCX_CPU1_LC_ART[index].PassToFailCount <> 0) or ;;
	(SCX_CPU1_LC_ART[index].ConsecutiveFailCount <> 0) or ;;
	(SCX_CPU1_LC_ART[index].CumulativeFailCount <> 0) or ;;
...
if (index < LC_MAX_ACTIONPOINTS-1) then
  ...
  write "  Index of failure       =", index
  write "  Action Results         =", SCX_CPU1_LC_ART[index].ActionResult
  write "  Current State          =", SCX_CPU1_LC_ART[index].CurrentState
  write "  Fail to Pass Count     =", SCX_CPU1_LC_ART[index].FailToPassCount
  write "  Pass to Fail Count     =", SCX_CPU1_LC_ART[index].PassToFailCount
  write "  Consecutive Fail Count =", SCX_CPU1_LC_ART[index].ConsecutiveFailCount
  write "  Cum Fail Count         =", SCX_CPU1_LC_ART[index].CumulativeFailCount
  write "  Cum RTS Count          =", SCX_CPU1_LC_ART[index].CumulativeRTSExecCount
...
/SCX_CPU1_EVS_EnaAppEVTType Application=LCAppName DEBUG
...
if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
  ...
  write "<!> Failed (1000;1003) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ",LC_NOOP_INF_EID, "."
  ut_setrequirements LC_1000, "F"
  ut_setrequirements LC_1003, "F"
endif
...
write ";*********************************************************************"
ut_setupevents "SCX", "CPU1", {LCAppName}, LC_LEN_ERR_EID, "ERROR", 1

local errcnt = SCX_CPU1_LC_CMDEC + 1

;; CPU1 is the default
rawcmd = "18a4c000000200BD"

ut_sendrawcmd "SCX_CPU1_LC", (rawcmd)

ut_tlmwait SCX_CPU1_LC_CMDEC, {errcnt}
if (UT_TW_Status = UT_Success) then
  write "<*> Passed (1002;1004) - Command Rejected Counter incremented."
  ut_setrequirements LC_1002, "P"
  ut_setrequirements LC_1004, "P"
else
  write "<!> Failed (1002;1004) - Command Rejected Counter did not increment as expected."
  ut_setrequirements LC_1002, "F"
  ut_setrequirements LC_1004, "F"
endif
...
if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
  ...
  write "<!> Failed (1004) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ",LC_LEN_ERR_EID, "."
  ut_setrequirements LC_1004, "F"
endif
...
/SCX_CPU1_TST_LC_SETCOUNTERS

wait 10

if (SCX_CPU1_LC_CMDPC <> 0) and (SCX_CPU1_LC_CMDEC <> 0) and ;;
   (SCX_CPU1_LC_PASSRTSCNT <> 0) and (SCX_CPU1_LC_APSAMPLECNT <> 0) and ;;
   (SCX_CPU1_LC_MONMSGCNT <> 0) and (SCX_CPU1_LC_RTSCNT <> 0) then
  ut_setupevents "SCX", "CPU1", {LCAppName}, LC_RESET_DBG_EID, "DEBUG", 1 
  /SCX_CPU1_LC_RESETCTRS
  wait 5

  ut_tlmwait SCX_CPU1_LC_CMDPC, 0
  if (UT_TW_Status = UT_Success) then
    write "<*> Passed (1001;1003) - Valid Command Counter was reset."
    ut_setrequirements LC_1001, "P"
    ut_setrequirements LC_1003, "P"
  else
    write "<!> Failed (1001;1003) - Valid Command Counter was NOT reset (",ut_tw_status,")."
    ut_setrequirements LC_1001, "F"
    ut_setrequirements LC_1003, "F"
  endif

  ut_tlmwait SCX_CPU1_LC_CMDEC, 0
  if (UT_TW_Status = UT_Success) then
  ...
  ut_tlmwait SCX_CPU1_LC_PASSRTSCNT, 0
  if (UT_TW_Status = UT_Success) then
  ...
  ut_tlmwait SCX_CPU1_LC_APSAMPLECNT, 0
  if (UT_TW_Status = UT_Success) then
  ...
  ut_tlmwait SCX_CPU1_LC_MONMSGCNT, 0
  if (UT_TW_Status = UT_Success) then
  ...
  ut_tlmwait SCX_CPU1_LC_RTSCNT, 0
  if (UT_TW_Status = UT_Success) then
  ...
  if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
    ...
    write "<!> Failed (1001) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ",LC_RESET_DBG_EID, "."
    ut_setrequirements LC_1001, "F"
    ut_setrequirements LC_1003, "F"
  endif
...
write ";*********************************************************************"
ut_setupevents "SCX", "CPU1", {LCAppName}, LC_LCSTATE_INF_EID, "INFO", 1

State = LC_STATE_ACTIVE

ut_sendcmd "SCX_CPU1_LC_SETLCSTATE NewLCState=State"
 
if (ut_sc_status = UT_SC_Success) and (SCX_CPU1_LC_CURLCSTATE=LC_STATE_ACTIVE) then
...
if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
  write "<*> Passed (1003;4000) - Event message ",SCX_CPU1_find_event[1].eventid, " received"
  ut_setrequirements LC_1003, "P"
  ut_setrequirements LC_4000, "P"
else
  write "<!> Failed (1003;4000) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ",LC_LCSTATE_INF_EID, "."
  ut_setrequirements LC_1003, "F"
  ut_setrequirements LC_4000, "F"
endif
...
if (ut_sc_status = UT_SC_Success) and (SCX_CPU1_LC_CURLCSTATE=LC_STATE_PASSIVE) then
...
if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
  write "<*> Passed (1003;4001) - Event message ",SCX_CPU1_find_event[1].eventid, " received"
  ut_setrequirements LC_1003, "P"
  ut_setrequirements LC_4001, "P"
else
  write "<!> Failed (1003;4001) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ",LC_LCSTATE_INF_EID, "."
  ut_setrequirements LC_1003, "F"
  ut_setrequirements LC_4001, "F"
endif
...
if (ut_sc_status = UT_SC_Success) and (SCX_CPU1_LC_CURLCSTATE=LC_STATE_DISABLED) then
...
if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
  write "<*> Passed (1003;4002) - Event message ",SCX_CPU1_find_event[1].eventid, " received"
  ut_setrequirements LC_1003, "P"
  ut_setrequirements LC_4002, "P"
else
  write "<!> Failed (1003;4002) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ",LC_LCSTATE_INF_EID, "."
  ut_setrequirements LC_1003, "F"
  ut_setrequirements LC_4002, "F"
endif
...
write ";*********************************************************************"
ut_setupevents "SCX", "CPU1", {LCAppName}, LC_LCSTATE_ERR_EID, "ERROR", 1
...
if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
  ...
  write "<!> Failed (1004) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ", LC_LCSTATE_ERR_EID, "."
  ut_setrequirements LC_1004, "F"
endif
...
write ";*********************************************************************"
ut_setupevents "SCX", "CPU1", {LCAppName}, LC_APSTATE_INF_EID, "INFO", 1
index = 0
State = LC_APSTATE_ACTIVE
ut_sendcmd "SCX_CPU1_LC_SETAPSTATE APNumber=index NewAPState=State"

if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
  write "<*> Passed (1003;4003) - Event message ",SCX_CPU1_find_event[1].eventid, " received"
  ut_setrequirements LC_1003, "P"
  ut_setrequirements LC_4003, "P"
else
  write "<!> Failed (1003;4003) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ",LC_APSTATE_INF_EID, "."
  ut_setrequirements LC_1003, "F"
...
for rdlindex = 0 to ADTentries-1 do
  if (rdlindex = 0) then
    if (SCX_CPU1_LC_ART[rdlindex].CurrentState <> LC_APSTATE_ACTIVE) then
      expectedResults = FALSE
      break
    endif
  elseif (SCX_CPU1_LC_ART[rdlindex].CurrentState <> LC_APSTATE_DISABLED) then
    expectedResults = FALSE
    break
  endif
enddo
...
if (expectedResults = TRUE) then
  ...
  write "<!> Failed (1003;4003) - AP #", rdlindex, " was not set as expected. Actual State = ",p@SCX_CPU1_LC_ART[rdlindex].CurrentState
  ut_setrequirements LC_1003, "F"
  ut_setrequirements LC_4003, "F"
endif
...
if (SCX_CPU1_find_event[1].num_found_messages = ADTentries - 1) then
  write "<*> Passed (1003;4003) - The correct number of Event messages were received."
    ut_setrequirements LC_1003, "P"
    ut_setrequirements LC_4003, "P"
else
  write "<!> Failed (1003;4003) - Did not rcv correct number of Event messages. Expected ",ADTentries - 1, " rcv'd ", SCX_CPU1_find_event[1].num_found_messages
  ut_setrequirements LC_1003, "F"
  ut_setrequirements LC_4003, "F"
endif 
...
if (expectedResults = TRUE) then
  ...
  write "<!> Failed (1003;4003) - AP #", rdlindex, " was not set to Active as expected. Actual State = ",p@SCX_CPU1_LC_ART[rdlindex].CurrentState
  ut_setrequirements LC_1003, "F"
  ut_setrequirements LC_4003, "F"
endif
...
if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
  write "<*> Passed (1003;4005) - Event message ",SCX_CPU1_find_event[1].eventid, " received"
  ut_setrequirements LC_1003, "P"
  ut_setrequirements LC_4005, "P"
else
  write "<!> Failed (1003;4005) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ",LC_APSTATE_INF_EID, "."
  ut_setrequirements LC_1003, "F"
...
for rdlindex = 0 to ADTentries-1 do
  if (rdlindex = 0) then
    if (SCX_CPU1_LC_ART[rdlindex].CurrentState <> LC_APSTATE_PASSIVE) then
      expectedResults = FALSE
      break
    endif
  elseif (SCX_CPU1_LC_ART[rdlindex].CurrentState <> LC_APSTATE_ACTIVE) then
    expectedResults = FALSE
    break
  endif
enddo
...
if (expectedResults = TRUE) then
  ...
  write "<!> Failed (1003;4005) - AP #", rdlindex, " was not set to Active as expected. Actual State = ",p@SCX_CPU1_LC_ART[rdlindex].CurrentState
  ut_setrequirements LC_1003, "F"
  ut_setrequirements LC_4005, "F"
endif
...
if (expectedResults = TRUE) then
  ...
  write "<!> Failed (1003;4003) - AP #", rdlindex, " was not set to Passive as expected. State = ",p@SCX_CPU1_LC_ART[rdlindex].CurrentState
  ut_setrequirements LC_1003, "F"
  ut_setrequirements LC_4003, "F"
endif
...
if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
  write "<*> Passed (1003;4007) - Event message ",SCX_CPU1_find_event[1].eventid, " received"
  ut_setrequirements LC_1003, "P"
  ut_setrequirements LC_4007, "P"
else
  write "<!> Failed (1003;4007) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ",LC_APSTATE_INF_EID, "."
  ut_setrequirements LC_1003, "F"
...
for rdlindex = 0 to ADTentries-1 do
  if (rdlindex = 0) then
    if (SCX_CPU1_LC_ART[rdlindex].CurrentState <> LC_APSTATE_DISABLED) then
      expectedResults = FALSE
      break
    endif
  elseif (SCX_CPU1_LC_ART[rdlindex].CurrentState <> LC_APSTATE_PASSIVE) then
    expectedResults = FALSE
    break
  endif
enddo
...
if (expectedResults = TRUE) then
  ...
  write "<!> Failed (1003;4007) - AP #", rdlindex, " was not set to Active as expected. Actual State = ",p@SCX_CPU1_LC_ART[rdlindex].CurrentState
  ut_setrequirements LC_1003, "F"
  ut_setrequirements LC_4007, "F"
endif
...
if (expectedResults = TRUE) then
  ...
  write "<!> Failed (1003;4003) - AP #", rdlindex, " was not set to Disabled as expected. State = ",p@SCX_CPU1_LC_ART[rdlindex].CurrentState
  ut_setrequirements LC_1003, "F"
  ut_setrequirements LC_4003, "F"
endif
...
if (expectedResults = TRUE) then
  ...
  write "<!> Failed (1003;4003) - AP #", rdlindex, " was not set to Active as expected. State = ",p@SCX_CPU1_LC_ART[rdlindex].CurrentState
  ut_setrequirements LC_1003, "F"
  ut_setrequirements LC_4003, "F"
endif
...
write ";*********************************************************************"
ut_setupevents "SCX", "CPU1", {LCAppName}, LC_APSTATE_NEW_ERR_EID, "ERROR", 1

errcnt = SCX_CPU1_LC_CMDEC + 1

;; This does not have to be a raw command since there are no restrictions on
;; the NewAPState argument
ut_sendcmd "SCX_CPU1_LC_SETAPSTATE APNumber=index NewAPState=4"
...
if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
  ...
  write "<!> Failed (1004) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ", LC_APSTATE_NEW_ERR_EID, "."
  ut_setrequirements LC_1004, "F"
endif
...
write ";*********************************************************************"
ut_setupevents "SCX", "CPU1", {LCAppName}, LC_APSTATE_CURR_ERR_EID, "ERROR", 1
...
if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
  ...
  write "<!> Failed (1004) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ", LC_APSTATE_CURR_ERR_EID, "."
  ut_setrequirements LC_1004, "F"
endif
...
write ";*********************************************************************"
State = LC_APSTATE_ACTIVE
ut_sendcmd "SCX_CPU1_LC_SETAPSTATE APNumber=0 NewAPState=State"
ut_sendcmd "SCX_CPU1_LC_SETAPSTATE APNumber=5 NewAPState=State"
ut_sendcmd "SCX_CPU1_LC_SETAPSTATE APNumber=8 NewAPState=State"
ut_sendcmd "SCX_CPU1_LC_SETAPSTATE APNumber=10 NewAPState=State"
State = LC_APSTATE_PASSIVE
ut_sendcmd "SCX_CPU1_LC_SETAPSTATE APNumber=1 NewAPState=State" 
ut_sendcmd "SCX_CPU1_LC_SETAPSTATE APNumber=4 NewAPState=State"
ut_sendcmd "SCX_CPU1_LC_SETAPSTATE APNumber=6 NewAPState=State"
ut_sendcmd "SCX_CPU1_LC_SETAPSTATE APNumber=7 NewAPState=State"
State = LC_APSTATE_DISABLED
ut_sendcmd "SCX_CPU1_LC_SETAPSTATE APNumber=2 NewAPState=State"
ut_sendcmd "SCX_CPU1_LC_SETAPSTATE APNumber=3 NewAPState=State"
ut_sendcmd "SCX_CPU1_LC_SETAPSTATE APNumber=9 NewAPState=State"
ut_sendcmd "SCX_CPU1_LC_SETAPSTATE APNumber=11 NewAPState=State"
...
State = LC_APSTATE_ACTIVE
ut_sendcmd "SCX_CPU1_LC_SETAPSTATE APNumber=0xFFFF NewAPState=State"
CmdStatus = CMDSUCCESS

if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
  write "<*> Passed (1003;4004) - Event message ",SCX_CPU1_find_event[1].eventid, " received"
  ut_setrequirements LC_1003, "P"
  ut_setrequirements LC_4004, "P"
else
  write "<!> Failed (1003;4004) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ",LC_APSTATE_INF_EID, "."
  ut_setrequirements LC_1003, "F"
...
for index = 0 to ADTentries-1 do
  if (SCX_CPU1_LC_ART[index].CurrentState <> LC_APSTATE_ACTIVE) then
     CmdStatus = CMDFAIL
     break
  endif
enddo
for index = ADTentries to LC_MAX_ACTIONPOINTS-1 do
  if (SCX_CPU1_LC_ART[index].CurrentState <> LC_ACTION_NOT_USED) then
     CmdStatus = CMDFAIL
     break
  endif
enddo
...
if (ut_sc_status = UT_SC_Success) and (CmdStatus = CMDSUCCESS) then
  ...
  write "<*> AP State = ", SCX_CPU1_LC_ART[index].CurrentState 
  ut_setrequirements LC_1003, "F"
  ut_setrequirements LC_4004, "F"
endif
...
if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
  write "<*> Passed (1003;4006) - Event message ",SCX_CPU1_find_event[1].eventid, " received"
  ut_setrequirements LC_1003, "P"
  ut_setrequirements LC_4006, "P"
else
  write "<!> Failed (1003;4006) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ",LC_APSTATE_INF_EID, "."
  ut_setrequirements LC_1003, "F"
...
for index = 0 to ADTentries-1 do
  if (SCX_CPU1_LC_ART[index].CurrentState <> LC_APSTATE_PASSIVE) then
     CmdStatus = CMDFAIL
     break
  endif
enddo
...
if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
  write "<*> Passed (1003;4008) - Event message ",SCX_CPU1_find_event[1].eventid, " received"
  ut_setrequirements LC_1003, "P"
  ut_setrequirements LC_4008, "P"
else
  write "<!> Failed (1003;4008) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ",LC_APSTATE_INF_EID, "."
  ut_setrequirements LC_1003, "F"
...
for index = 0 to ADTentries-1 do
  if (SCX_CPU1_LC_ART[index].CurrentState <> LC_APSTATE_DISABLED) then
     CmdStatus = CMDFAIL
     break
  endif
enddo
...
ut_sendcmd "SCX_CPU1_LC_SETAPSTATE APNumber=0xFFFF NewAPState=4"
...
ut_setupevents "SCX", "CPU1", {LCAppName}, LC_APOFF_CURR_ERR_EID, "ERROR", 1

/SCX_CPU1_LC_SETAPPERMOFF APNumber=0
...
if (UT_TW_Status = UT_Success) THEN
  ...
  write "<!> Failed (1004) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ",LC_APOFF_CURR_ERR_EID, "."
  ut_setrequirements LC_1004, "F"
endif
...
/SCX_CPU1_LC_SETAPPERMOFF APNumber=4
...
write ";  should pass."
write ";*********************************************************************"
ut_setupevents "SCX", "CPU1", {LCAppName}, LC_APOFF_INF_EID, "INFO", 1
rdlindex = 2

ut_sendcmd "SCX_CPU1_LC_SETAPPERMOFF APNumber=2"

if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
  write "<*> Passed (1003;4009) - Event message ",SCX_CPU1_find_event[1].eventid, " received"
  ut_setrequirements LC_1003, "P"
  ut_setrequirements LC_4009, "P"
else
  write "<!> Failed (1003;4009) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ",LC_APOFF_INF_EID, "."
  ut_setrequirements LC_1003, "F"
...
if (ut_sc_status = UT_SC_Success) and (SCX_CPU1_LC_ART[rdlindex].CurrentState = LC_APSTATE_PERMOFF) then
...
/SCX_CPU1_LC_SETAPPERMOFF APNumber=12
...
/SCX_CPU1_LC_SETAPSTATE APNumber=2 NewAPState=State
...
/SCX_CPU1_LC_SETAPPERMOFF APNumber=2

ut_tlmwait SCX_CPU1_LC_CMDEC, {errcnt}
if (UT_TW_Status = UT_Success) AND ;;
   (SCX_CPU1_find_event[1].num_found_messages = 1) then
  write "<*> Passed (1004) - Command Rejected Counter incremented and Event message ",SCX_CPU1_find_event[1].eventid, " received."
  ut_setrequirements LC_1004, "P"
elseif (SCX_CPU1_LC_CMDEC <> errcnt) then
  write "<!> Failed (1004) - Command Rejected Counter did not increment as expected."
elseif (SCX_CPU1_find_event[1].num_found_messages <> 1) then
  write "<!> Failed (1004) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ",LC_APOFF_CURR_ERR_EID, "."
  ut_setrequirements LC_1004, "F"
endif
...
for rdlindex = 0 to LC_MAX_WATCHPOINTS-1 do
  if (scx_cpu1_LC_WRT[rdlindex].EvaluationCount = 0) or ;;
     (scx_cpu1_LC_WRT[rdlindex].FalsetoTrueCount  = 0) or ;;
     (scx_cpu1_LC_WRT[rdlindex].ConsectiveTrueCount  = 0) or ;;
     (scx_cpu1_LC_WRT[rdlindex].CumulativeTrueCount  = 0) or ;;
     (scx_cpu1_LC_WRT[rdlindex].SecFtoTTimeStamp  = 0) or ;;
     (scx_cpu1_LC_WRT[rdlindex].SubSecFtoTTimeStamp  = 0) or ;;
     (scx_cpu1_LC_WRT[rdlindex].SecTtoFTimeStamp  = 0) or ;;
     (scx_cpu1_LC_WRT[rdlindex].SubSecTtoFTimeStamp  = 0) then
    write ";*********************************************************************"
...
if (CmdStatus = CMDSUCCESS) then
  ...
  ut_setupevents "SCX", "CPU1", {LCAppName}, LC_WPSTATS_INF_EID, "INFO", 1

  /SCX_CPU1_LC_RESETWPSTATS WPNumber=0
  /SCX_CPU1_LC_RESETWPSTATS WPNumber=maxwp
  /SCX_CPU1_LC_RESETWPSTATS WPNumber=midWP

  ut_tlmwait  SCX_CPU1_find_event[1].num_found_messages, 3
  if (UT_TW_Status = UT_Success) THEN
    write "<*> Passed (1003;4012) - Event messages ",SCX_CPU1_find_event[1].eventid, " received"
    ut_setrequirements LC_1003, "P"
    ut_setrequirements LC_4012, "P"
  else
    write "<!> Failed (1003;4012) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ",LC_WPSTATS_INF_EID, " 3 times."
    ut_setrequirements LC_1003, "F"
    ut_setrequirements LC_4012, "F"
  endif
  ...
  for rdlindex = 0 to LC_MAX_WATCHPOINTS-1 do
    if  (rdlindex = 0) or (rdlindex = midWP) or (rdlindex = maxwp) then
      if (scx_cpu1_LC_WRT[rdlindex].EvaluationCount <> 0) and ;;
	 (scx_cpu1_LC_WRT[rdlindex].FalsetoTrueCount  <> 0) and ;;
	 (scx_cpu1_LC_WRT[rdlindex].ConsectiveTrueCount  <> 0) and ;;
	 (scx_cpu1_LC_WRT[rdlindex].CumulativeTrueCount  <> 0) and ;;
	 (scx_cpu1_LC_WRT[rdlindex].SecFtoTTimeStamp  <> 0) and ;;
	 (scx_cpu1_LC_WRT[rdlindex].SubSecFtoTTimeStamp  <> 0) and ;;
	 (scx_cpu1_LC_WRT[rdlindex].SecTtoFTimeStamp  <> 0) and ;;
	 (scx_cpu1_LC_WRT[rdlindex].SubSecTtoFTimeStamp  <> 0) then 
	...
	expectedResults = FALSE
	 ...
	 (scx_cpu1_LC_WRT[rdlindex].SubSecTtoFTimeStamp  = 0) then 
        ...
        write "Evaluation Count        = ", scx_cpu1_LC_WRT[rdlindex].EvaluationCount
        write "False to True Count     = ", scx_cpu1_LC_WRT[rdlindex].FalsetoTrueCount
        write "Consecutive True Count  = ", scx_cpu1_LC_WRT[rdlindex].ConsectiveTrueCount 
        write "Cumulative True Count   = ", scx_cpu1_LC_WRT[rdlindex].CumulativeTrueCount
        write "Time Stamp (seconds)    = ", scx_cpu1_LC_WRT[rdlindex].SecFtoTTimeStamp
        write "Time Stamp (subseconds) = ", scx_cpu1_LC_WRT[rdlindex].SubSecFtoTTimeStamp
        write "Time Stamp (seconds)    = ", scx_cpu1_LC_WRT[rdlindex].SecTtoFTimeStamp 
        write "Time Stamp (subseconds) = ", scx_cpu1_LC_WRT[rdlindex].SubSecTtoFTimeStamp
        ut_setrequirements LC_1003, "F"
        ut_setrequirements LC_2004, "F"
        ut_setrequirements LC_4012, "F"
	expectedResults = FALSE
...
for rdlindex = 0 to LC_MAX_ACTIONPOINTS-1 do
  if (scx_cpu1_LC_ART[rdlindex].FailToPassCount = 0) or ;;
     (scx_cpu1_LC_ART[rdlindex].PassToFailCount = 0) or ;;
     (scx_cpu1_LC_ART[rdlindex].ConsecutiveFailCount = 0) or ;;
     (scx_cpu1_LC_ART[rdlindex].CumulativeFailCount = 0) or ;;
     (scx_cpu1_LC_ART[rdlindex].CumulativeRTSExecCount = 0) then 
    write ";*********************************************************************"
...
if (CmdStatus = CMDSUCCESS) then
  ...
  ut_setupevents "SCX", "CPU1", {LCAppName}, LC_APSTATS_INF_EID, "INFO", 1

  /SCX_CPU1_LC_RESETAPSTATS APNumber=0
  /SCX_CPU1_LC_RESETAPSTATS APNumber=maxap
  /SCX_CPU1_LC_RESETAPSTATS APNumber=midAP

  ut_tlmwait  SCX_CPU1_find_event[1].num_found_messages, 3
  if (UT_TW_Status = UT_Success) THEN
    write "<*> Passed (1003;4010) - Event message ",SCX_CPU1_find_event[1].eventid, " received"
    ut_setrequirements LC_1003, "P"
    ut_setrequirements LC_4010, "P"
  else
    write "<!> Failed (1003;4010) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ",LC_APOFF_INF_EID, "."
    ut_setrequirements LC_1003, "F"
    ut_setrequirements LC_4010, "F"
  endif
  ...
  for rdlindex = 0 to LC_MAX_ACTIONPOINTS-1 do
    if  (rdlindex = 0) or (rdlindex = midAP) or (rdlindex = maxap) then
      if (scx_cpu1_LC_ART[rdlindex].FailToPassCount <> 0) and ;;
	 (scx_cpu1_LC_ART[rdlindex].PassToFailCount <> 0) and ;;
	 (scx_cpu1_LC_ART[rdlindex].ConsecutiveFailCount <> 0) and ;;
	 (scx_cpu1_LC_ART[rdlindex].CumulativeFailCount <> 0) and ;;
	 (scx_cpu1_LC_ART[rdlindex].CumulativeRTSExecCount <> 0) then 
        write "<!> Failed (1003;3006;4010) - Reset AP Statistics did not clear index (", rdlindex, ") as expected."
        write "Fail to Pass Count     = ",scx_cpu1_LC_ART[rdlindex].FailToPassCount
        write "Pass to Fail Count     = ",scx_cpu1_LC_ART[rdlindex].PassToFailCount
        write "Consecutive Fail Count = ",scx_cpu1_LC_ART[rdlindex].ConsecutiveFailCount
        write "Cumulative Fail Count  = ",scx_cpu1_LC_ART[rdlindex].CumulativeFailCount
        write "Cumulative RTS Count   = ",scx_cpu1_LC_ART[rdlindex].CumulativeRTSExecCount 
        ut_setrequirements LC_1003, "F"
...
if (CmdStatus = CMDSUCCESS) then
  ut_setupevents "SCX", "CPU1", {LCAppName}, LC_WPSTATS_INF_EID, "INFO", 1
  cmdctr = SCX_CPU1_LC_CMDPC + 1
  /SCX_CPU1_LC_RESETWPSTATS WPNumber=0xFFFF
  ut_tlmwait  SCX_CPU1_find_event[1].num_found_messages, 1

  if (UT_TW_Status = UT_Success) THEN
    write "<*> Passed (1003;4013) - Event message ",SCX_CPU1_find_event[1].eventid, " received"
    ut_setrequirements LC_1003, "P"
    ut_setrequirements LC_4013, "P"
  else       
    write "<!> Failed (1003;4013) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ",LC_APOFF_INF_EID, "."
    ut_setrequirements LC_1003, "F"
    ut_setrequirements LC_4013, "F"
  endif

  s get_tbl_to_cvt(ramDir, WRTTblName, "A", "cpu1_dumpwrt", hostCPU, wrtAppId)
  wait 5

  ut_tlmwait SCX_CPU1_LC_CMDPC, {cmdctr}
...
if (CmdStatus = CMDSUCCESS) then
  ut_setupevents "SCX", "CPU1", {LCAppName}, LC_APSTATS_INF_EID, "INFO", 1
  cmdctr = SCX_CPU1_LC_CMDPC + 1
  /SCX_CPU1_LC_RESETAPSTATS APNumber=0xFFFF
  ut_tlmwait SCX_CPU1_find_event[1].num_found_messages, 1
 
  if (UT_TW_Status = UT_Success) THEN
    write "<*> Passed (1003;4011) - Event message ",SCX_CPU1_find_event[1].eventid, " received"
    ut_setrequirements LC_1003, "P"
    ut_setrequirements LC_4011, "P"
  else       
    write "<!> Failed (1003;4011) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ",LC_APOFF_INF_EID, "."
    ut_setrequirements LC_1003, "F"
    ut_setrequirements LC_4011, "F"
  endif
...
write ";*********************************************************************"
ut_setupevents "SCX", "CPU1", {LCAppName}, LC_APSTATE_APNUM_ERR_EID, "ERROR", 1

errcnt = SCX_CPU1_LC_CMDEC + 1
State = LC_APSTATE_ACTIVE 

/SCX_CPU1_LC_SETAPSTATE APNumber=180 NewAPState=State
ut_tlmwait SCX_CPU1_LC_CMDEC, {errcnt}
if (UT_TW_Status = UT_Success) then
  write "<*> Passed (1002;1004) - Command Rejected Counter incremented."
  ut_setrequirements LC_1002, "P"
...
if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
  ...
  write "<!> Failed (1004) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ", LC_APSTATE_APNUM_ERR_EID, "."
  ut_setrequirements LC_1004, "F"
endif
...
/SCX_CPU1_LC_SETAPSTATE APNumber=222 NewAPState=State
...
/SCX_CPU1_LC_SETAPSTATE APNumber=254 NewAPState=State
...
write ";*********************************************************************"
ut_setupevents "SCX", "CPU1", {LCAppName}, LC_APOFF_APNUM_ERR_EID, "ERROR", 1

errcnt = SCX_CPU1_LC_CMDEC + 1

/SCX_CPU1_LC_SETAPPERMOFF APNumber=260
...
if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
  ...
  write "<!> Failed (1004) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ", LC_APOFF_APNUM_ERR_EID, "."
  ut_setrequirements LC_1004, "F"
endif
...
write ";*********************************************************************"
ut_setupevents "SCX", "CPU1", {LCAppName}, LC_APSAMPLE_APNUM_ERR_EID, "ERROR", 1

;; CPU1 is the default
rawcmd = "18a6c00000090000fffc000000000000"
...
if (UT_TW_Status = UT_Success) then
  ...
  write "<!> Failed (1004) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ", LC_APSAMPLE_APNUM_ERR_EID, "."
  ut_setrequirements LC_1004, "F"
endif
...
for rdlindex = 0 to LC_MAX_ACTIONPOINTS-1 do
  if (SCX_CPU1_LC_ART[rdlindex].CurrentState = LC_ACTION_NOT_USED) then
    apNotUsed = rdlIndex
    break
  endif
enddo

ut_setupevents "SCX", "CPU1", {LCAppName}, LC_APSAMPLE_CURR_ERR_EID, "ERROR", 1

/SCX_CPU1_TST_LC_SENDSAMPLE StartAP=apNotUsed EndAP=apNotUsed UpdateAge=0
...
if (UT_TW_Status = UT_Success) then
  ...
  write "<!> Failed (1004) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ", LC_APSAMPLE_CURR_ERR_EID, "."
  ut_setrequirements LC_1004, "F"
endif
...
;; Find an AP that is Perm Off
for rdlindex = 0 to LC_MAX_ACTIONPOINTS-1 do
  if (p@SCX_CPU1_LC_ART[rdlindex].CurrentState = "Permanently Disabled") then
    apPermOff = rdlIndex
    break
  endif
enddo
...
/SCX_CPU1_TST_LC_SENDSAMPLE StartAP=apPermOff EndAP=apPermOff UpdateAge=0
...
write ";*********************************************************************"
ut_setupevents "SCX", "CPU1", {LCAppName}, LC_WPSTATS_WPNUM_ERR_EID, "ERROR", 1

errcnt = SCX_CPU1_LC_CMDEC + 1

/SCX_CPU1_LC_RESETWPSTATS WPNumber=250
...
if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
  ...
  write "<!> Failed (1004) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ", LC_WPSTATS_WPNUM_ERR_EID, "."
  ut_setrequirements LC_1004, "F"
endif
...
write ";*********************************************************************"
ut_setupevents "SCX", "CPU1", {LCAppName}, LC_APSTATS_APNUM_ERR_EID, "ERROR", 1

errcnt = SCX_CPU1_LC_CMDEC + 1

/SCX_CPU1_LC_RESETAPSTATS APNumber=250
...
if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
  ...
  write "<!> Failed (1004) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ", LC_APSTATS_APNUM_ERR_EID, "."
  ut_setrequirements LC_1004, "F"
endif
...
write ";*********************************************************************"
ut_setupevents "SCX", "CPU1", {LCAppName}, LC_CC_ERR_EID, "ERROR", 1
...
if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
  ...
  write "<!> Failed (1004) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ",LC_CC_ERR_EID, "."
  ut_setrequirements LC_1004, "F"
endif
...
write ";*********************************************************************"
ut_setupevents "SCX", "CPU1", {LCAppName}, LC_APSAMPLE_LEN_ERR_EID, "ERROR", 1
...
if (UT_TW_Status = UT_Success) THEN
  ...
  write "<!> Failed (1004) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ",LC_APSAMPLE_LEN_ERR_EID, "."
  ut_setrequirements LC_1004, "F"
endif
...
write ";*********************************************************************"
ut_setupevents "SCX", "CPU1", {LCAppName}, LC_HKREQ_LEN_ERR_EID, "ERROR", 1
...
if (UT_TW_Status = UT_Success) THEN
  ...
  write "<!> Failed (1004) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ",LC_HKREQ_LEN_ERR_EID, "."
  ut_setrequirements LC_1004, "F"
endif
...
write ";*********************************************************************"
write ";  End procedure SCX_CPU1_lc_gencmds                                  "
write ";*********************************************************************"
ENDPROC
