...
;s ftp_file("CF:0", "lc_def_wdt1.tbl", wdtFileName, hostCPU, "P")

; Load the file created above
s load_table("lc_def_wdt1.tbl",hostCPU)
wait 5

/SCX_CPU1_TBL_VALIDATE INACTIVE VTABLENAME=WDTTblName
wait 5

/SCX_CPU1_TBL_ACTIVATE ATableName=WDTTblName
wait 5
...
;s ftp_file("CF:0", "lc_def_adt1b.tbl", adtFileName, hostCPU, "P")

; Load the file created above
s load_table("lc_def_adt1b.tbl",hostCPU)
wait 5

/SCX_CPU1_TBL_VALIDATE INACTIVE VTABLENAME=ADTTblName
wait 5

/SCX_CPU1_TBL_ACTIVATE ATableName=ADTTblName
wait 5
...
write "; Skipping Steps 1.3 and 1.4 since the apps are contained in the startup script"
;write ";*********************************************************************"
...
;; 51 is because the APs are not used and not measured
  ...
  if (wpindex < WPACKED) OR (apindex < APACKED) then
    ...
    write "  CURLCSTATE        = ", p@SCX_CPU1_LC_CURLCSTATE
    ...
    if (wpindex < WPACKED) then
      write "  WP Packed Results = ", %hex(SCX_CPU1_LC_WRRESULTS[wpindex].WPByteEntry.AllStatus,2)
    endif
    if (apindex < APACKED) then
      write "  AP Packed Results = ", %hex(SCX_CPU1_LC_ARRESULTS[apindex].APByteEntry.AllStatus,2)
    endif
...
;; Dump the WRT
s get_tbl_to_cvt(ramDir, WRTTblName, "A", "cpu1_dumpwrt", hostCPU, wrtPktId)
wait 5
...
if (index < LC_MAX_WATCHPOINTS-1) then
  ...
  write " WatchResults        = ", p@SCX_CPU1_LC_WRT[index].WatchResults
...
;; Dump the ART
s get_tbl_to_cvt(ramDir, ARTTblName, "A", "cpu1_dumpart", hostCPU, artPktId)
wait 5 
...
if (index < LC_MAX_ACTIONPOINTS-1) then
  ...
  write "  Action Results         = ", p@SCX_CPU1_LC_ART[index].ActionResult
  write "  Current State          = ", p@SCX_CPU1_LC_ART[index].CurrentState
...
if (ut_sc_status = UT_SC_Success) and ;;
   (SCX_CPU1_LC_CURLCSTATE=LC_STATE_PASSIVE) then
...
s scx_cpu1_lc_sendmonpackets("2.3")
...
/SCX_CPU1_TST_LC_SENDSAMPLE StartAP=0 EndAP=11 UpdateAge=0
ut_tlmupdate SCX_CPU1_TST_LC_CMDPC
...
write ";*********************************************************************"
if (SCX_CPU1_LC_PASSRTSCNT = 0) AND ;;
   (SCX_CPU1_LC_CURLCSTATE=LC_STATE_PASSIVE) AND ;;
   (SCX_CPU1_LC_APSAMPLECNT = 12) AND ;;
   (SCX_CPU1_LC_MONMSGCNT = 20) AND (SCX_CPU1_LC_RTSCNT = 0) THEN
  ;; for the WP measured results for wpindex 1-7 should be 0 (false), wpindex 8 which is 240 (0xF0), rest 255 (0xFF)
  for wpindex = 1 to WPACKED do
    if (wpindex <=7) then
      if (SCX_CPU1_LC_WRRESULTS[wpindex].WPByteEntry.AllStatus <> 0) then
        break
      endif
    elseif  (wpindex = 8) then
      if (SCX_CPU1_LC_WRRESULTS[wpindex].WPByteEntry.AllStatus <> 240) then
        break
      endif
    else
      if (SCX_CPU1_LC_WRRESULTS[wpindex].WPByteEntry.AllStatus <> 255) then
  ...
  for apindex = 1 to APACKED do
    if (apindex <= APResultEntries) then
      if (SCX_CPU1_LC_ARRESULTS[apindex].APByteEntry.AllStatus <> 68) then
        break                                   
      endif
    else
      if (SCX_CPU1_LC_ARRESULTS[apindex].APByteEntry.AllStatus <> 51) then
...
for index = 0 to LC_MAX_WATCHPOINTS-1 do
  if (index < WDTentries) then
    write " FtoT Type = ", p@SCX_CPU1_LC_WRT[index].FtoTDataType
    write " TtoF Type = ", p@SCX_CPU1_LC_WRT[index].TtoFDataType
    if  (SCX_CPU1_LC_WRT[index].WatchResults <> LC_WATCH_FALSE) or ;;
	(SCX_CPU1_LC_WRT[index].EvaluationCount <> 1) or ;;
	(SCX_CPU1_LC_WRT[index].FalsetoTrueCount <> 0) or ;;
	(SCX_CPU1_LC_WRT[index].ConsectiveTrueCount <> 0) or ;;
	(SCX_CPU1_LC_WRT[index].CumulativeTrueCount <> 0) or ;;
	(SCX_CPU1_LC_WRT[index].FtoTValue.Unsigned32 <> 0) then
;	(SCX_CPU1_LC_WRT[index].FtoTValue.Unsigned32 <> 0) or ;;
;	(SCX_CPU1_LC_WRT[index].TtoFValue.Unsigned32 <> 0) then
      break
    endif
...
for index = 0 to LC_MAX_ACTIONPOINTS-1 do
  if (index < ADTentries) then
    if  (SCX_CPU1_LC_ART[index].ActionResult <> LC_ACTION_PASS) or ;;
	(SCX_CPU1_LC_ART[index].CurrentState <> LC_APSTATE_ACTIVE) or ;;
	(SCX_CPU1_LC_ART[index].FailToPassCount <> 0) or ;;
	(SCX_CPU1_LC_ART[index].PassToFailCount <> 0) or ;;
	(SCX_CPU1_LC_ART[index].ConsecutiveFailCount <> 0) or ;;
	(SCX_CPU1_LC_ART[index].CumulativeFailCount <> 0) or ;;
...
s scx_cpu1_lc_sendmonpackets("3.3")
...
write ";*********************************************************************"
if (SCX_CPU1_LC_PASSRTSCNT = 0) AND ;;
   (SCX_CPU1_LC_CURLCSTATE=LC_STATE_DISABLED) AND ;;
   (SCX_CPU1_LC_APSAMPLECNT = 12) AND ;;
   (SCX_CPU1_LC_MONMSGCNT = 20) AND (SCX_CPU1_LC_RTSCNT = 0) THEN
;; for the WP measured results for wpindex 1-7 should be 0 (false), wpindex 8 which is 240 (0xF0), rest 255 (0xFF)
  for wpindex = 1 to WPACKED do
...
for index = 0 to LC_MAX_WATCHPOINTS-1 do
  if (index < WDTentries) then
	...
	(SCX_CPU1_LC_WRT[index].ConsectiveTrueCount <> 0) or ;;
	(SCX_CPU1_LC_WRT[index].CumulativeTrueCount <> 0) or ;;
	(SCX_CPU1_LC_WRT[index].FtoTValue.Unsigned32 <> 0) ;;
	then
;	(SCX_CPU1_LC_WRT[index].FtoTValue.Unsigned32 <> 0) or ;;
;	(SCX_CPU1_LC_WRT[index].TtoFValue.Unsigned32 <> 0) then
      break
...
s scx_cpu1_lc_sendmonpackets("4.3")
...
if (SCX_CPU1_LC_PASSRTSCNT = 0) AND ;;
   (SCX_CPU1_LC_CURLCSTATE=LC_STATE_ACTIVE) AND ;;
   (SCX_CPU1_LC_APSAMPLECNT = 12) AND ;;
   (SCX_CPU1_LC_MONMSGCNT = 40) AND (SCX_CPU1_LC_RTSCNT = 0) THEN
;; for the WP measured results for wpindex 1-7 should be 0 (false), wpindex 8 which is 240 (0xF0), rest 255 (0xFF)
  for wpindex = 1 to WPACKED do
    if (wpindex <=7) then
      if (SCX_CPU1_LC_WRRESULTS[wpindex].WPByteEntry.AllStatus <> 0) then
...
;; rest should be 51 (0x33) since they are not used and not measured
  for apindex = 1 to APACKED do
    if (apindex <= APResultEntries) then
      if (SCX_CPU1_LC_ARRESULTS[apindex].APByteEntry.AllStatus <> 204) then
        break                                   
      endif
    else
      if (SCX_CPU1_LC_ARRESULTS[apindex].APByteEntry.AllStatus <> 51) then
...
for index = 0 to LC_MAX_WATCHPOINTS-1 do
  if (index < WDTentries) then
    if  (SCX_CPU1_LC_WRT[index].WatchResults <> LC_WATCH_FALSE) or ;;
	(SCX_CPU1_LC_WRT[index].EvaluationCount <> 2) or ;;
	(SCX_CPU1_LC_WRT[index].FalsetoTrueCount <> 0) or ;;
	(SCX_CPU1_LC_WRT[index].ConsectiveTrueCount <> 0) or ;;
	(SCX_CPU1_LC_WRT[index].CumulativeTrueCount <> 0) or ;;
	(SCX_CPU1_LC_WRT[index].FtoTValue.UnSigned32 <> 0) ;;
...
s scx_cpu1_lc_sendmonpackets("5.3")
...
if (SCX_CPU1_LC_PASSRTSCNT = 0) AND ;;
   (SCX_CPU1_LC_CURLCSTATE=LC_STATE_ACTIVE) AND ;;
   (SCX_CPU1_LC_APSAMPLECNT = 24) AND ;;
   (SCX_CPU1_LC_MONMSGCNT = 60) AND (SCX_CPU1_LC_RTSCNT = 0) THEN
;; for the WP measured results for wpindex 1-7 should be 0 (false), wpindex 8 which is 240 (0xF0), rest 255 (0xFF)
  for wpindex = 1 to WPACKED do
    if (wpindex <=7) then
      if (SCX_CPU1_LC_WRRESULTS[wpindex].WPByteEntry.AllStatus <> 0) then
...
for index = 0 to LC_MAX_WATCHPOINTS-1 do
  if (index < WDTentries) then
    if  (SCX_CPU1_LC_WRT[index].WatchResults <> LC_WATCH_FALSE) or ;;
	(SCX_CPU1_LC_WRT[index].EvaluationCount <> 3) or ;;
	(SCX_CPU1_LC_WRT[index].FalsetoTrueCount <> 0) or ;;
	(SCX_CPU1_LC_WRT[index].ConsectiveTrueCount <> 0) or ;;
	(SCX_CPU1_LC_WRT[index].CumulativeTrueCount <> 0) or ;;
	(SCX_CPU1_LC_WRT[index].FtoTValue.Unsigned32 <> 0) ;;
...
s scx_cpu1_lc_sendmonpackets("6.2")
...
if (SCX_CPU1_LC_PASSRTSCNT = 0) AND ;;
   (SCX_CPU1_LC_CURLCSTATE=LC_STATE_ACTIVE) AND ;;
   (SCX_CPU1_LC_APSAMPLECNT = 36) AND ;;
   (SCX_CPU1_LC_MONMSGCNT = 80) AND (SCX_CPU1_LC_RTSCNT = 0) THEN
;; for the WP measured results for wpindex 1-7 should be 0 (false), wpindex 8 which is 240 (0xF0), rest 255 (0xFF)
  for wpindex = 1 to WPACKED do
    if (wpindex <=7) then
      if (SCX_CPU1_LC_WRRESULTS[wpindex].WPByteEntry.AllStatus <> 0) then
...
;; rest should be 51 (0x33) since they are not used and not measured
  for apindex = 1 to APACKED do
    if (apindex <= APResultEntries) then
      if (SCX_CPU1_LC_ARRESULTS[apindex].APByteEntry.AllStatus <> 136) then
        break                                   
      endif
    else
      if (SCX_CPU1_LC_ARRESULTS[apindex].APByteEntry.AllStatus <> 51) then
...
for index = 0 to LC_MAX_WATCHPOINTS-1 do
  if (index < WDTentries) then
    if  (SCX_CPU1_LC_WRT[index].WatchResults <> LC_WATCH_FALSE) or ;;
	(SCX_CPU1_LC_WRT[index].EvaluationCount <> 4) or ;;
	(SCX_CPU1_LC_WRT[index].FalsetoTrueCount <> 0) or ;;
	(SCX_CPU1_LC_WRT[index].ConsectiveTrueCount <> 0) or ;;
	(SCX_CPU1_LC_WRT[index].CumulativeTrueCount <> 0) or ;;
	(SCX_CPU1_LC_WRT[index].FtoTValue.Unsigned32 <> 0) ;;
...
for index = 0 to LC_MAX_ACTIONPOINTS-1 do
  if (index < ADTentries) then
    if  (SCX_CPU1_LC_ART[index].ActionResult <> LC_ACTION_PASS) or ;;
	(SCX_CPU1_LC_ART[index].CurrentState <> LC_APSTATE_PASSIVE) or ;;
	(SCX_CPU1_LC_ART[index].FailToPassCount <> 0) or ;;
	(SCX_CPU1_LC_ART[index].PassToFailCount <> 0) or ;;
	(SCX_CPU1_LC_ART[index].ConsecutiveFailCount <> 0) or ;;
	(SCX_CPU1_LC_ART[index].CumulativeFailCount <> 0) or ;;
...
/SCX_CPU1_LC_SETAPPERMOFF APNumber=3
...
if (CmdStatus = CMDSUCCESS) then
  index = index + 1
  if (SCX_CPU1_LC_ART[index].CurrentState <> LC_APSTATE_PERMOFF) then
    CmdStatus = CMDFAIL
  endif
endif
if (CmdStatus = CMDSUCCESS) then
...
s scx_cpu1_lc_sendmonpackets("7.2")
...
if (SCX_CPU1_LC_PASSRTSCNT = 0) AND ;;
   (SCX_CPU1_LC_CURLCSTATE=LC_STATE_ACTIVE) AND ;;
   (SCX_CPU1_LC_APSAMPLECNT = 44) AND ;;
   (SCX_CPU1_LC_MONMSGCNT = 100) AND (SCX_CPU1_LC_RTSCNT = 0) THEN
;; for the WP measured results for wpindex 1-7 should be 0 (false), wpindex 8 which is 240 (0xF0), rest 255 (0xFF)
  for wpindex = 1 to WPACKED do
    if (wpindex <=7) then
      if (SCX_CPU1_LC_WRRESULTS[wpindex].WPByteEntry.AllStatus <> 0) then
...
;; rest should be 51 (0x33) since they are not used and not measured
  CmdStatus = CMDSUCCESS
  apindex = 1
  if (SCX_CPU1_LC_ARRESULTS[apindex].APByteEntry.AllStatus <> 132) then
    CmdStatus = CMDFAIL                                  
  endif

  if (CMDStatus = CMDSUCCESS) then
    apindex = apindex + 1
    if (SCX_CPU1_LC_ARRESULTS[apindex].APByteEntry.AllStatus <> 12) then
      CmdStatus = CMDFAIL                                  
    endif
  endif
...
for index = 0 to LC_MAX_WATCHPOINTS-1 do
  if (index < WDTentries) then
    if  (SCX_CPU1_LC_WRT[index].WatchResults <> LC_WATCH_FALSE) or ;;
	(SCX_CPU1_LC_WRT[index].EvaluationCount <> 5) or ;;
	(SCX_CPU1_LC_WRT[index].FalsetoTrueCount <> 0) or ;;
	(SCX_CPU1_LC_WRT[index].ConsectiveTrueCount <> 0) or ;;
	(SCX_CPU1_LC_WRT[index].CumulativeTrueCount <> 0) or ;;
	(SCX_CPU1_LC_WRT[index].FtoTValue.Unsigned32 <> 0) ;;
...
if (CmdStatus = CMDSUCCESS) then
  index = index + 1
  if (SCX_CPU1_LC_ART[index].ActionResult <> LC_ACTION_PASS) or ;;
     (SCX_CPU1_LC_ART[index].CurrentState <> LC_APSTATE_PERMOFF) or ;;
     (SCX_CPU1_LC_ART[index].FailToPassCount <> 0) or ;;
     (SCX_CPU1_LC_ART[index].PassToFailCount <> 0) or ;;
     (SCX_CPU1_LC_ART[index].ConsecutiveFailCount <> 0) or ;;
     (SCX_CPU1_LC_ART[index].CumulativeFailCount <> 0) or ;;
...
write ";*********************************************************************"
write ";  End procedure SCX_CPU1_lc_monitoring                              "
write ";*********************************************************************"
ENDPROC
