PROC $sc_$cpu_lc_tabletesting
;*****************************************************************************
;  Test Name:  lc_tabletesting
...
local cfe_requirements[0 .. ut_req_array_size] = ["LC_1003","LC_2004", ;;
"LC_2005","LC_3006","LC_3007","LC_4000","LC_4001","LC_4002","LC_4004", ;;
"LC_8000","LC_9000","LC_9001","LC_9002","LC_9005","LC_9006","LC_9007"]
...
s get_tbl_to_cvt(ramDir, WRTTblName, "A", "$cpu_dumpwrt", hostCPU, wpAPID)
wait 5
...
s get_tbl_to_cvt(ramDir, ARTTblName, "A", "$cpu_dumpart", hostCPU, apAPID)
wait 5 
...
s load_table("lc_def_wdt2.tbl",hostCPU)
...
if (index < LC_MAX_WATCHPOINTS-1) then
  ...
  write " Consecutive True    = ", $SC_$CPU_LC_WRT[index].ConsectiveTrueCount
  write " Cum True Count      = ", $SC_$CPU_LC_WRT[index].CumulativeTrueCount
  write " F to T Value        = ", %hex($SC_$CPU_LC_WRT[index].FtoTValue.unsigned32,2)
  write " T to F Value        = ", %hex($SC_$CPU_LC_WRT[index].TtoFValue.unsigned32,2)
...
s load_table("lc_def_adt2.tbl",hostCPU)
...
;; Send the command to load the table
s load_table("lc_def_wdt2a.tbl",hostCPU)
s load_table("lc_def_adt2a.tbl",hostCPU)
...
;; Send the command to load the table
s load_table("lc_def_wdt3.tbl",hostCPU)
s load_table("lc_def_adt3.tbl",hostCPU)
...
write ";*********************************************************************"
write ";  Step 5.2: Replace the default load files with the files that contain"
write ";  validation errors for the WDT and ADT tables."
write ";*********************************************************************"
s ftp_file(defaultTblDir, "lc_def_wdt3.tbl", wdtFileName, hostCPU, "P")
s ftp_file(defaultTblDir, "lc_def_adt3.tbl", adtFileName, hostCPU, "P")

write ";*********************************************************************"
write ";  Step 5.3: Perform an Application reset"
write ";*********************************************************************"
...
write ";*********************************************************************"
write ";  Step 5.4: Start the Limit Checker (LC) Application and verify the "
write ";  WDT table fails validation and the LC application does not start. "
write ";********************************************************************"
ut_setupevents "$SC", "$CPU", {LCAppName}, LC_WDT_LOAD_ERR_EID, "ERROR", 1
ut_setupevents "$SC", "$CPU", {LCAppName}, LC_WDTVAL_ERR_EID, "ERROR", 2
ut_setupevents "$SC", "$CPU", {LCAppName}, LC_WDTVAL_INF_EID, "INFO", 3
...
ut_tlmwait $SC_$CPU_find_event[1].num_found_messages, 1
IF (UT_TW_Status = UT_Success) THEN
  write "<*> Passed - WDT Load Error Event Message received."
else
  write "<!> Failed - WDT Load Error Event Message not received."
endif

ut_tlmwait $SC_$CPU_find_event[2].num_found_messages, 1
IF (UT_TW_Status = UT_Success) THEN
  write "<*> Passed - WDT Validation Error Event Message received."
else
  write "<!> Failed - WDT Validation Error Event Message not received."
endif

ut_tlmwait $SC_$CPU_find_event[3].num_found_messages, 1
IF (UT_TW_Status = UT_Success) THEN
  write "<*> Passed - WDT Validation Info Event Message received."
else
  write "<!> Failed - WDT Validation Info Event Message not received."
endif

wait 5

write ";*********************************************************************"
write ";  Step 5.5: Perform a Power-On reset"
write ";*********************************************************************"
/$SC_$CPU_ES_POWERONRESET
wait 10
...
write ";*********************************************************************"
write ";  Step 5.6: Start the Limit Checker (LC) Application and verify the "
write ";  WDT table fails validation.  "
write ";********************************************************************"
ut_setupevents "$SC", "$CPU", {LCAppName}, LC_WDT_LOAD_ERR_EID, "ERROR", 1
ut_setupevents "$SC", "$CPU", {LCAppName}, LC_WDTVAL_ERR_EID, "ERROR", 2
ut_setupevents "$SC", "$CPU", {LCAppName}, LC_WDTVAL_INF_EID, "INFO", 3
...
;; restore the default WDT file
s ftp_file(defaultTblDir, "lc_def_wdt1.tbl", wdtFileName, hostCPU, "P")

wait 5

write ";*********************************************************************"
write ";  Step 5.7: Perform a Processor Reset"
write ";*********************************************************************"
...
write ";*********************************************************************"
write ";  Step 5.8: Start the Limit Checker (LC) Application and verify the "
write ";  ADT and WDT tables fail validation.  "
write ";********************************************************************"
ut_setupevents "$SC", "$CPU", {LCAppName}, LC_ADT_LOAD_ERR_EID, "ERROR", 1
ut_setupevents "$SC", "$CPU", {LCAppName}, LC_ADTVAL_ERR_EID, "ERROR", 2
ut_setupevents "$SC", "$CPU", {LCAppName}, LC_ADTVAL_INF_EID, "INFO", 3
...
ut_tlmwait $SC_$CPU_find_event[1].num_found_messages, 1
IF (UT_TW_Status = UT_Success) THEN
  write "<*> Passed - ADT Load Error Event Message received."
else
  write "<!> Failed - ADT Load Error Event Message not received."
endif

ut_tlmwait $SC_$CPU_find_event[2].num_found_messages, 1
IF (UT_TW_Status = UT_Success) THEN
  write "<*> Passed - ADT Validation Error Event Message received."
else
  write "<!> Failed - ADT Validation Error Event Message not received."
endif

ut_tlmwait $SC_$CPU_find_event[3].num_found_messages, 1
IF (UT_TW_Status = UT_Success) THEN
  write "<*> Passed - ADT Validation Info Event Message received."
else
  write "<!> Failed - ADT Validation Info Event Message not received."
endif

;; restore the default ADT file
s ftp_file(defaultTblDir, "lc_def_adt1.tbl", adtFileName, hostCPU, "P")
...
write ";*********************************************************************"
write ";  End procedure $SC_$CPU_lc_tabletesting "
write ";*********************************************************************"
ENDPROC
