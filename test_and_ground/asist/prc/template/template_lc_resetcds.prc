PROC $sc_$cpu_lc_resetcds
;*******************************************************************************
;  Test Name:  lc_resetcds
...
;	performed. Finally, this test verifies that the LC application restores
...
;                 g) The cumulative count of the RTS executions
;                 h) Total number of event messages sent
;   LC9004	Upon a cFE Processor Reset or LC Application Reset, if the 
;		<PLATFORM_DEFINED> Save Critical Data parameter is set to YES,
;		LC shall restore the following data:
...
;   LC9004.1.1	If the platform defined Default Reset State indicates to use the
;		state of the LC Application prior to the reset, LC shall set
;		the state of the LC Application to the state restored from the
;		CDS
;   LC9004.2	If LC determines the Critical Data is invalid, LC shall
;		perform the same initialization as a cFE Power-On (see LC9000,
;		LC9001 and LC9002)
...
;               defined in the WDT.
;   LC9007.1	For a cFE Processor Reset, if the Save Critical Data parameter
;		is YES, LC shall subscribe to the messages defined in the WDT
;		restored from the CDS
;   LC9007.2	For an LC Application Reset, if the Save Critical Data
;		parameter is YES, LC shall subscribe to the messages defined
...
;			  testing
;       lc_wdt2          Sets up the Watchpoint Definition table files for
;			  testing
;       lc_adt2          Sets up the Actionpoint Definition table files for
;			  testing
;
...
local cfe_requirements[0 .. ut_req_array_size] = ["LC_1003","LC_2004", ;;
    "LC_3006","LC_4000","LC_4004","LC_8000","LC_9000","LC_9001", ;;
    "LC_9002","LC_9004","LC_9004.1","LC_9004.1.1","LC_9004.2", ;;
    "LC_9005","LC_9006","LC_9007","LC_9007.1","LC_9007.2"]
...
s $sc_$cpu_lc_sendpackets("2.3",1)
...
if (LC_STATE_WHEN_CDS_RESTORED = LC_STATE_FROM_CDS) then
  if ($SC_$CPU_LC_CURLCSTATE = LC_STATE_ACTIVE) then
    ...
    ut_setrequirements LC_90041, "P"
    ut_setrequirements LC_900411, "P"
  else
    write "<!> Failed (9004.1;9004.1.1) - LC Application State NOT set properly. Expected Active. State = ",p@$SC_$CPU_LC_CURLCSTATE
    ut_setrequirements LC_90041, "F"
    ut_setrequirements LC_900411, "F"
  endif
else
...
s $sc_$cpu_lc_sendpackets("2.11",2)
...
s get_tbl_to_cvt(ramDir, "LC.LC_WDT", "A", "$cpu_lc_wdt.dat", hostCPU, wdtAPID)
wait 5
...
s ftp_file(defaultTblDir, "lc_def_wdt2.tbl", wdtFileName, hostCPU, "P")

s $SC_$CPU_lc_adt2

s ftp_file(defaultTblDir, "lc_def_adt2.tbl", adtFileName, hostCPU, "P")
...
;; 51 is because the APs are not used and not measured
  ...
  if (wpindex < WPACKED) OR (apindex < APACKED) then
    write "<!> Failed (9000;9004.2) - WP or AP Housekeeping telemetry NOT initialized at startup."
    write "  CMDPC             = ", $SC_$CPU_LC_CMDPC 
    write "  CMDEC             = ", $SC_$CPU_LC_CMDEC 
    write "  PASSRTSCNT        = ", $SC_$CPU_LC_PASSRTSCNT
    write "  CURLCSTATE        = ", p@$SC_$CPU_LC_CURLCSTATE
    ...
    ut_setrequirements LC_9000, "F"
    ut_setrequirements LC_90042, "F"
  else
    write "<*> Passed (9000;9004.2) - Housekeeping telemetry initialized properly."
    ut_setrequirements LC_9000, "P"
    ut_setrequirements LC_90042, "P"
  endif  
else
  write "<!> Failed (9000;9004.2) - Housekeeping telemetry NOT initialized at startup."
  write "  CMDPC             = ", $SC_$CPU_LC_CMDPC 
  write "  CMDEC             = ", $SC_$CPU_LC_CMDEC 
  write "  PASSRTSCNT        = ", $SC_$CPU_LC_PASSRTSCNT
  write "  CURLCSTATE        = ", p@$SC_$CPU_LC_CURLCSTATE
...
if (index < LC_MAX_WATCHPOINTS-1) then
  write "<!> Failed (9001;9004.2;9005) - Watchpoint Results Table NOT initialized at startup."
  write " Index of failure    = ", index
  write " WatchResults        = ", p@$SC_$CPU_LC_WRT[index].WatchResults
  write " Evaluation Count    = ", $SC_$CPU_LC_WRT[index].EvaluationCount
  write " False to True Count = ", $SC_$CPU_LC_WRT[index].FalsetoTrueCount
  ...
  write "<*> Passed (9001;9004.2;9005) - Watchpoint Results Table initialized properly."
  ut_setrequirements LC_9001, "P"  
  ut_setrequirements LC_90042, "P"
  ut_setrequirements LC_9005, "P"  
endif
...
if (index < LC_MAX_ACTIONPOINTS-1) then
  write "<!> Failed (9002;9004.2;9006) - Actionpoint Results Table NOT initialized at startup."
  write "  Index of failure       = ", index
  write "  Action Results         = ", p@$SC_$CPU_LC_ART[index].ActionResult
  write "  Current State          = ", p@$SC_$CPU_LC_ART[index].CurrentState
  write "  Fail to Pass Count     = ", $SC_$CPU_LC_ART[index].FailToPassCount
  ...
  write "<*> Passed (9002;9004.2;9006) - Actionpoint Results Table initialized properly."
  ut_setrequirements LC_9002, "P"  
  ut_setrequirements LC_90042, "P"
  ut_setrequirements LC_9006, "P"  
endif
...
write ";*********************************************************************"
write ";  End procedure $SC_$CPU_lc_resetcds"
write ";*********************************************************************"
ENDPROC
